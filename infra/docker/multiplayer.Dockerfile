# NIGHTCELL 7 — multiplayer service.
#
...
# services/multiplayer/railway.json; railpack cannot auto-detect a start command in
...
RUN pnpm --filter @nightcell7/service-multiplayer build
...
COPY packages ./packages
COPY services ./services
RUN pnpm install --frozen-lockfile --prod --filter @nightcell7/service-multiplayer...

COPY --from=build /app/services/multiplayer/dist ./services/multiplayer/dist

USER node
CMD ["node", "services/multiplayer/dist/index.js"]
