...
# services/worker/railway.json; railpack cannot auto-detect a start command in
...
RUN pnpm --filter @nightcell7/service-worker build
...
COPY packages ./packages
COPY services ./services
RUN pnpm install --frozen-lockfile --prod --filter @nightcell7/service-worker...
