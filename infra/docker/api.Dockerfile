...
# services/api/railway.json; railpack cannot auto-detect a start command in
...
RUN pnpm --filter @nightcell7/service-api build
...
COPY packages ./packages
COPY services ./services
RUN pnpm install --frozen-lockfile --prod --filter @nightcell7/service-api...
