...
# services/cron/railway.json; railpack cannot auto-detect a start command in
...
RUN pnpm --filter @nightcell7/service-cron build
...
COPY packages ./packages
COPY services ./services
RUN pnpm install --frozen-lockfile --prod --filter @nightcell7/service-cron...

COPY --from=build /app/services/cron/dist ./services/cron/dist

USER node
CMD ["node", "services/cron/dist/index.js"]
