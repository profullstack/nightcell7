...
export const serviceBuild: Options = {
  ...
  // Native/dynamic-require modules must NOT be inlined. `pg` does a dynamic
  // `require("pg-native")` that throws "Dynamic require is not supported" the
  // moment it lands in an ESM bundle. Bundling a workspace package pulls its
  // transitive deps in with it, so these have to be named explicitly even
  // where a service does not list them directly.
