...
def bind():
    for mat in list(bpy.data.materials):
        ...
        if not spec:
            ...
            tex=nodes.new('ShaderNodeTexImage');tex.image=bpy.data.images.load(str(ROOT/'apps/game/public/assets/textures'/f'{mat.name}_albedo.webp'));links.new(tex.outputs['Color'],bs.inputs['Base Color'])
            orm=nodes.new('ShaderNodeTexImage');orm.image=bpy.data.images.load(str(ROOT/'apps/game/public/assets/textures'/f'{mat.name}_orm.webp'));orm.image.colorspace_settings.name='Non-Color'
        ...
        normal=nodes.new('ShaderNodeTexImage');normal.image=bpy.data.images.load(str(ROOT/'apps/game/public/assets/textures'/f"{spec['detailSource']}_normal.webp"));normal.image.colorspace_settings.name='Non-Color';links.new(mapping.outputs[0],normal.inputs['Vector'])
...
def main(entry):
    ...
    if name in ['character','fighter_insurgent','fighter_soldier']:
        bpy.context.scene.frame_set(1);bpy.context.view_layer.update()
        socket=bpy.data.objects.get('SOCKET_WEAPON')
        if socket:
            before=set(bpy.context.scene.objects)
            weapon='wep_smg' if name=='fighter_insurgent' else 'wep_rifle'
    ...
    elif name.startswith('wep_') or 'carbine' in name:
