...
try {
  ...
  await mkdir(root + "build/iron-rain", { recursive: true });
  await page.evaluate(() => {
    const e = window.__NC7_DEV__.engine;
    window.__IR_LOOPS = [...e._activeRenderLoops];
    e.stopRenderLoop();
  });
  await page.screenshot({ path: root + "build/iron-rain/demo-combat.png", timeout: 120000 });
  await page.evaluate(() => {
    for (const loop of window.__IR_LOOPS) window.__NC7_DEV__.engine.runRenderLoop(loop);
    delete window.__IR_LOOPS;
  });
  if (pointerLockAdapter)
    await page.evaluate(() =>
  ...
  assert.ok([...models].every((n) => n.startsWith("m3_") || n === "m2_carbine_fp.glb"));
  ...
  await writeFile(root + "build/iron-rain/browser-smoke.json", JSON.stringify(report, null, 2));
